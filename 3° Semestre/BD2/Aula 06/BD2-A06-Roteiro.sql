...
SELECT 
  f.tituloPortugues,
  SUM(v.valorIngresso) AS total_vendido
FROM venda v
JOIN sessao s ON v.idSessao = s.id
JOIN filme f ON s.idFilme = f.id
WHERE v.idTipoPagto = 2 -- parcelado
...
SELECT COUNT(DISTINCT v.idUsuario) AS usuarios_vista
FROM venda v
...
CREATE PROCEDURE alteraValorIngresso (IN novo_valor DECIMAL(10,2))
BEGIN
...
CREATE PROCEDURE pagaParcelasUsuario(IN idUser INT)
BEGIN
  UPDATE parcela p
  JOIN venda v ON p.idVenda = v.id
  SET p.situacao = 'PAGA'
  WHERE v.idUsuario = idUser;
END$$

DELIMITER ;

-- Exemplo de uso:
CALL pagaParcelasUsuario(2);
...
CREATE PROCEDURE parcelasUsuario(IN idUser INT)
BEGIN
...
CREATE PROCEDURE visualizaVendas(IN tipo_pagto INT)
BEGIN
  SELECT 
    tipoPagto.nome,
    SUM(venda.valorIngresso) AS total_vendido
  FROM venda
  JOIN tipoPagto ON tipoPagto.id = venda.idTipoPagto
  WHERE venda.idTipoPagto = tipo_pagto
  GROUP BY tipoPagto.nome;
END$$
...
CREATE PROCEDURE cinemasPorCidade(IN nomeCidade VARCHAR(45))
BEGIN
  SELECT c.nomeFantasia, c.endereco, c.bairro
  FROM cinema c
...
CREATE PROCEDURE atualizaGenero(
  IN genero_antigo VARCHAR(45),
  IN genero_novo VARCHAR(45)
)
BEGIN
  DECLARE id_antigo INT;
  DECLARE id_novo INT;

  SELECT id INTO id_antigo FROM genero WHERE nome = genero_antigo;
  SELECT id INTO id_novo FROM genero WHERE nome = genero_novo;

  UPDATE filme SET idGenero = id_novo WHERE idGenero = id_antigo;
END$$
...
CREATE PROCEDURE filmesGeneroDuracao(
  IN nomeGenero VARCHAR(45),
  IN minDuracao INT
)
BEGIN
  ...
  FROM filme f
  JOIN genero g ON f.idGenero = g.id
  WHERE g.nome = nomeGenero AND f.duracao > minDuracao;
END$$

DELIMITER ;

-- Exemplo:
CALL filmesGeneroDuracao('Drama', 120);
...
SELECT 
  ...
  s.publico,
  c.capacidade,
  ROUND((s.publico / c.capacidade) * 100, 2) AS ocupacao_percentual
FROM sessao s
...
CREATE VIEW resumo_vendas_filme AS
SELECT 
  f.tituloPortugues,
...
CREATE VIEW ocupacao_cinema AS
SELECT 
  c.nomeFantasia,
  ROUND(AVG(s.publico / c.capacidade) * 100, 2) AS media_ocupacao_percentual
FROM sessao s
JOIN cinema c ON s.idCinema = c.id
